...
「サンプルデータを試す」ボタンを押すと、実際のレース名入りのサンプルデータ（[`public/sample.csv`](./public/sample.csv)、15件）がワンタップでアプリに取り込まれます（ファイルのダウンロードは発生しません。iOS Safariのダウンロードプレビュー画面が固まる問題を避けるため、直接読み込む方式にしています）。生データを確認したい場合は [`public/sample.csv`](./public/sample.csv) を参照してください。
